...
func runAnsibleCheckWithOutput(playbook string, inventory string, targetHosts string, log zerolog.Logger) (bool, bool, string, string) {
	...
	// Log a clean summary instead of the full verbose output
	logSummary(playbook, output, log)
	...
	if strings.Contains(output, "changed=0") {
		log.Info().Str("playbook", playbook).Msg("No drift - no changes detected")
	...
	if strings.Contains(output, "changed=") && !strings.Contains(output, "changed=0") {
		log.Debug().Str("playbook", playbook).Msg("Checking if changes are ignorable")

		// Check if all changes are ignorable
		if areChangesIgnorable(output) {
			log.Info().Str("playbook", playbook).Msg("No drift - ignorable changes only")
			return false, false, "ok", ""
		}

		// Changes are not ignorable, this is true drift
		log.Warn().Str("playbook", playbook).Msg("Drift detected - running remediation")
...
// logSummary logs a clean, concise summary of the Ansible check output
func logSummary(playbook, output string, log zerolog.Logger) {
	// Extract the PLAY RECAP line which contains the summary
	...
	if strings.Contains(output, "changed=") {
		// Find the changed count in the output
